...
The function is called in EAs when the [Trade](/en/docs/runtime/event_fire#trade) event occurs. The function is meant for processing changes in order, position and trade lists.
...
When handling an order, a trade server sends the terminal a message about the incoming [Trade](/en/docs/runtime/event_fire#trade) event. To retrieve relevant data on orders and trades from history, it is necessary to perform a trading history request using [HistorySelect()](/en/docs/trading/historyselect) first.
...
[OnTrade()](/en/docs/event_handlers/ontrade) handler is called after the appropriate [OnTradeTransaction()](/en/docs/event_handlers/ontradetransaction) calls. In general, there is no exact correlation in the number of OnTrade () and OnTradeTransaction () calls. One OnTrade() call corresponds to one or several OnTradeTransaction calls.
...
[OrderSend](/en/docs/trading/ordersend), [OrderSendAsync](/en/docs/trading/ordersendasync), [OnTradeTransaction](/en/docs/event_handlers/ontradetransaction),  [Client terminal events](/en/docs/runtime/event_fire)
