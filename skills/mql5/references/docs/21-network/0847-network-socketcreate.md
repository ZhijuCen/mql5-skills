...
In case of a successful socket creation, return its handle, otherwise [INVALID_HANDLE](/en/docs/constants/namedconstants/otherconstants).

Notes

To free up computer memory from an unused socket, call [SocketClose](/en/docs/network/socketclose) for it.

You can create a maximum of 128 sockets from one MQL5 program. If the limit is exceeded, the error 5271 (ERR_NETSOCKET_TOO_MANY_OPENED) is written to [_LastError](/en/docs/predefined/_lasterror).
