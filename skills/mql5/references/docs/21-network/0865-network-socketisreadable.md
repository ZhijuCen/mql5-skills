...
Before calling [SocketRead](/en/docs/network/socketread), check if the socket features data for reading. Otherwise, if there are no data, the [SocketRead](/en/docs/network/socketread) function waits for data within timeout_ms delaying the program execution.
