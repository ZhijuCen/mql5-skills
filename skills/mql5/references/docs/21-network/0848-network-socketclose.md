...
[in]  Handle of a socket to be closed. The handle is returned by the [SocketCreate](/en/docs/network/socketcreate) function. When an incorrect handle is passed, the error 5270 (ERR_NETSOCKET_INVALIDHANDLE) is written to [_LastError](/en/docs/predefined/_lasterror).
...
If a connection via [SocketConnect](/en/docs/network/socketconnect) was previously created for a socket, it is discontinued.
