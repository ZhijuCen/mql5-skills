...
All requests to execute trade operations are sent as a structure of a trade request [MqlTradeRequest](/en/docs/constants/structures/mqltraderequest) using function [OrderSend()](/en/docs/trading/ordersend). The function execution result is placed to structure [MqlTradeResult](/en/docs/constants/structures/mqltraderesult), whose retcode field contains the trade server return code.
