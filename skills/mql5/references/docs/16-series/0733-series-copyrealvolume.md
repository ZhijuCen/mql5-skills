...
![When copying the yet unknown amount of data, it  is recommended to use  dynamic array  as a target array, because if the requested data count is less (or more) than the length of the target array, function  tries to reallocate the memory so that the requested data fit entirely.](pics/copyrealvolume.png)
...
Returns the copied data count or -1 in the case of  [error](/en/docs/constants/errorswarnings/errorcodes).
