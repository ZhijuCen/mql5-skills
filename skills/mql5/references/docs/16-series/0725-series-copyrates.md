...
Gets history data of [MqlRates](/en/docs/constants/structures/mqlrates) structure of a specified symbol-period in specified quantity into the rates_array array. The elements ordering of the copied data is from present to the past, i.e., starting position of 0 means the current bar.

![When copying the yet unknown amount of data, it  is recommended to use  dynamic array  as a target array, because if the requested data count is less (or more) than the length of the target array, function  tries to reallocate the memory so that the requested data fit entirely.](pics/copyrates.png)
...
[out]  Array of [MqlRates](/en/docs/constants/structures/mqlrates) type.

Return Value

Returns the number of copied elements or -1 in case of  an [error](/en/docs/constants/errorswarnings/errorcodes).
...
[Structures and Classes](/en/docs/basis/types/classes),  [TimeToString](/en/docs/convert/timetostring), [StringFormat](/en/docs/convert/stringformat)
