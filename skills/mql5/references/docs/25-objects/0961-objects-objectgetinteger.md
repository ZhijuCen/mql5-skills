...
The function returns the value of the corresponding object property. The object property must be of the [datetime, int, color, bool or char](/en/docs/basis/types/integer) type. There are 2 variants of the function.
...
For the second variant the function returns true, if this property is maintained and the value has been placed into the long_var variable, otherwise returns false. To read more about the [error](/en/docs/constants/errorswarnings/errorcodes) call [GetLastError()](/en/docs/check/getlasterror).
